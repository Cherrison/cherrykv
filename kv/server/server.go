...
import (
	...
	"github.com/cherrison/cherrykv/kv/transaction/latches"
	"github.com/cherrison/cherrykv/kv/transaction/mvcc"
...
var _ cherrykvpb.CherryKvServer = new(Server)
...
// The below functions are Server's gRPC API (implements CherryKvServer).
...
func (server *Server) Raft(stream cherrykvpb.CherryKv_RaftServer) error {
	return server.storage.(*raft_storage.RaftStorage).Raft(stream)
}

// Snapshot stream (tinykv <-> tinykv)
// Only used for RaftStorage, so trivially forward it.
func (server *Server) Snapshot(stream cherrykvpb.CherryKv_SnapshotServer) error {
