...
func TestBenchTest(t *testing.T)  {
	...
	cluster.MustPut([]byte("k1"), []byte("v1"))
	cluster.MustGet([]byte("k1"), []byte("v1"))
	cluster.MustPut([]byte("cherrisong"), []byte("i am from ocean university of china"))
	log.Error(string(cluster.Get([]byte("k1"))))
	log.Error(string(cluster.Get([]byte("cherrisong"))))
	cluster.ClearFilters()
}