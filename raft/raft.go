...
func (r *Raft) tick() {
	...
	case StateCandidate:
		r.tickElection()
	case StateLeader:
		//  leadTransferee 不是None 就处理Transfer请求 暂停leader原本的heartbeat
...
func (r *Raft) tickElection() {
	...
	if r.electionElapsed >= r.randomElectionTimeout {
		r.electionElapsed = 0
		r.Step(pb.Message{MsgType: pb.MessageType_MsgElection})
	}
}
...
func (r *Raft) handleTransferLeader(m pb.Message) {
	...
	if r.Prs[m.From].Match == r.RaftLog.LastIndex() {
		r.sendTimeoutNow(m.From)
	} else { // transferElapsed 设置为 0 循环检测日志是否同步， 同步后立即发送
		r.sendAppend(m.From)
	}
}
