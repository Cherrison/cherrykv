...
import (
	...
	pb "github.com/cherrison/cherrykv/proto/pkg/eraftpb"
)
...
func TestLeaderElection2AA(t *testing.T) {
	...
	for i, tt := range tests {
		tt.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
...
func TestLeaderCycle2AA(t *testing.T) {
	...
	for campaignerID := uint64(1); campaignerID <= 3; campaignerID++ {
		n.send(pb.Message{From: campaignerID, To: campaignerID, MsgType: pb.MessageType_MsgElection})
...
func TestLeaderElectionOverwriteNewerLogs2AB(t *testing.T) {
	...
	n.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
	sm1 := n.peers[1].(*Raft)
	...
	tests := []struct {
		*network
		...
		},
		{
			...
			[]pb.Message{
				{From: 1, To: 1, MsgType: pb.MessageType_MsgPropose, Entries: []*pb.Entry{{Data: []byte("somedata")}}},
				{From: 1, To: 2, MsgType: pb.MessageType_MsgElection},
...
func TestCommitWithoutNewTermEntry2AB(t *testing.T) {
	...
	// should be committed
	tt.send(pb.Message{From: 2, To: 2, MsgType: pb.MessageType_MsgElection})
...
func TestDuelingCandidates2AB(t *testing.T) {
	...
	nt.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
	nt.send(pb.Message{From: 3, To: 3, MsgType: pb.MessageType_MsgElection})
...
func TestCandidateConcede2AB(t *testing.T) {
	...
	tt.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
	tt.send(pb.Message{From: 3, To: 3, MsgType: pb.MessageType_MsgElection})
...
func testCandidateResetTerm(t *testing.T, mt pb.MessageType) {
	...
	nt.send(pb.Message{From: 2, To: 2, MsgType: pb.MessageType_MsgElection})
	nt.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
...
func TestCampaignWhileLeader2AA(t *testing.T) {
	...
	r.Step(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgElection})
...
func TestLeaderTransferReceiveHigherTermVote3A(t *testing.T) {
	...
	nt.send(pb.Message{From: 3, To: 1, MsgType: pb.MessageType_MsgTransferLeader})
	nt.send(pb.Message{From: 2, To: 2, MsgType: pb.MessageType_MsgElection, Index: 1, Term: 2})
...
func TestSplitVote2AA(t *testing.T) {
	...
	nt.isolate(1)
	nt.send([]pb.Message{
		{From: 2, To: 2, MsgType: pb.MessageType_MsgElection},
		{From: 3, To: 3, MsgType: pb.MessageType_MsgElection},
	}...)
...
func (nw *network) filter(msgs []pb.Message) []pb.Message {
	...
	for _, m := range msgs {
		...
		switch m.MsgType {
		case pb.MessageType_MsgElection:
			// hups never go over the network, so don't drop them but panic
			panic("unexpected MessageType_MsgElection")
		default:
